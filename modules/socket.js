      ...
      conn.query('SELECT * FROM users WHERE ?', {access_token: access_token}, function(errU, users){
        ...
        conn.query('SELECT * FROM customers WHERE ?',{user_id: users[0].id}, function(errC, customers){
          ...
          if(money >= total){
            ...
            ds.set(dataPayment.app_id+'-'+dataPayment.case_id, {product_id: dataPayment.product_id}, 'payjson');
          }
          else{
            ...
            var bill2 = {
              ...
              case_id: dataPayment.case_id,
              price: dataPayment.product_price,
              message: customers[0].fullname+' không đủ tiền mua sản phẩm '+dataPayment.product_name,
              status:0
            };
